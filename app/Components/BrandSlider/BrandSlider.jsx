...
const BrandSlider = () => {
  // Corrected the image path to be relative to the public folder
  const brandLogos = Array(15).fill('/assets/br1.png'); // Fixed image path
      ...
      <div className="overflow-hidden py-4">
            ...
            {brandLogos.concat(brandLogos).map((logo, index) => (
              <img
                ...
                onError={(e) => {
                  ...
                  e.currentTarget.src = 'https://placehold.co/100x50/e0e0e0/555?text=Logo+Err';
                }}
              />
            ))}
          </div>
        ...
        <div className="brand-logo-row mt-8">
          <div className="animate-slide-right">
            {/* Duplicate logos for the reverse animation effect */}
            {brandLogos.concat(brandLogos).map((logo, index) => (
              <img
                key={`row2-${index}`}
                src={logo}
